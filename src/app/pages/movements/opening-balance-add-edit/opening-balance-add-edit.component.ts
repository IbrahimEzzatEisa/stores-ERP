...
@Component({
  selector: 'app-opening-balance-add-edit',
  templateUrl: './opening-balance-add-edit.component.html'
})
export class OpeningBalanceAddEditComponent implements OnInit {
  ...
  ngOnInit() {
    this.permission = this.permissionsService.getPermission(PERMISSIONS.openingBalance);
  ...
  reset() {
    ...
    setTimeout(()=>{
      this.storeTrxes = new StoreTrxe({trxTypeId: 0});
      this.getNewId();
    }, 50);
    this.isEdit = false;
    this.rank = null;
